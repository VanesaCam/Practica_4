...
# ──  CONTENIDO ADMIN ──────────────────────────────────────────────────────
@course_bp.route('/admin/contenido/nuevo', methods=['GET', 'POST'])
@login_required
@solo_admin
def admin_crear_contenido():
    # Cargar módulos para el selector
    conn = __import__('models.database', fromlist=['get_connection']).get_connection()
    modulos = conn.execute('SELECT id, numero, titulo FROM modulos ORDER BY numero').fetchall()
    conn.close()
    ...
    if request.method == 'POST':
        ...
        if not titulo or not descripcion or not modulo_id:
            flash('Todos los campos son obligatorios.', 'error')
            return render_template('admin/nuevo_contenido.html', modulos=modulos)

        ok = crear_nuevo_material(modulo_id, titulo, descripcion)

        if ok:
            flash('¡Contenido creado exitosamente!', 'success')
            return redirect(url_for('course.admin_panel'))
        else:
            flash('Hubo un error al guardar en la base de datos.', 'error')
