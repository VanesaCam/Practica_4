...
def crear_nuevo_material(modulo_id, titulo, descripcion):
    """Inserta una nueva lección en la base de datos SQLite."""
    from models.database import get_connection
    try:
        conn = get_connection()
        # Calcular el siguiente número disponible para ese módulo
        result = conn.execute(
            'SELECT COALESCE(MAX(numero), 0) + 1 FROM lecciones WHERE modulo_id = ?',
            (modulo_id,)
        ).fetchone()
        siguiente_numero = result[0]

        conn.execute(
            'INSERT INTO lecciones (modulo_id, numero, titulo, contenido) VALUES (?, ?, ?, ?)',
            (modulo_id, siguiente_numero, titulo, descripcion)
        )
        conn.commit()
        conn.close()
        return True
